# Skein Android - Agent Guide

This document provides context, architectural insights, and development standards for AI agents working on the Skein Android codebase.

## 1. Project Overview

**Skein** is a decentralized, offline-first communication application focused on privacy and resilient mesh messaging (campus, field, and disaster-resilient scenarios). It uses mesh networking (primarily Bluetooth LE, with optional Tor/Arti and Nostr for internet-facing features) so peers can exchange messages without centralized chat servers.

Skein is based on the open-source bitchat-android mesh stack and evolved as an academic/engineering project under the Skein name. Do not invent a greenfield BLE mesh history; extend and document what the codebase already implements.
...
- **Naming**: Clear, descriptive names. Follow standard Android naming patterns (e.g., `*ViewModel`, `*Repository`, `*Screen`). Prefer `Skein*` product naming over legacy `Bitchat*` names when introducing or renaming public types.
...
4. **Scope discipline**: Product plan files such as `SkeinFINAL.md` may intentionally discuss the bitchat technical base; do not “rebrand away” that historical/engineering context unless the user asks.
