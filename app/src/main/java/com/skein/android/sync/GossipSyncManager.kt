package com.skein.android.sync

import android.util.Log
import com.skein.android.mesh.BluetoothPacketBroadcaster
...
class GossipSyncManager(
    ...
    interface Delegate {
        ...
        fun sendPacketToPeer(peerID: String, packet: SkeinPacket)
        fun signPacketForBroadcast(packet: SkeinPacket): SkeinPacket
    }
    ...
    private val messages = LinkedHashMap<String, SkeinPacket>()
    // - announcements: only keep latest per sender peerID
    private val latestAnnouncementByPeer = ConcurrentHashMap<String, Pair<String, SkeinPacket>>()
    ...
    fun start() {
        ...
        cleanupJob = scope.launch(Dispatchers.IO) {
            while (isActive) {
                try {
                    delay(com.skein.android.util.AppConstants.Sync.CLEANUP_INTERVAL_MS)
    ...
    fun onPublicPacketSeen(packet: SkeinPacket) {
        ...
        if (isBroadcastMessage) {
            ...
            if (age > com.skein.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS) {
                Log.d(TAG, "Ignoring stale ANNOUNCE (age=${age}ms > ${com.skein.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS}ms)")
                return
            }
    ...
    private fun sendRequestSync() {
        ...
        val packet = SkeinPacket(
            ...
            payload = payload,
            ttl = com.skein.android.util.AppConstants.SYNC_TTL_HOPS // neighbors only
        )
        // Sign and broadcast
    ...
    private fun sendRequestSyncToPeer(peerID: String) {
        ...
        val packet = SkeinPacket(
            ...
            timestamp = System.currentTimeMillis().toULong(),
            payload = payload,
            ttl = com.skein.android.util.AppConstants.SYNC_TTL_HOPS // neighbor only
        )
    ...
    fun handleRequestSync(fromPeerID: String, request: RequestSyncPacket) {
        ...
        for ((_, pair) in latestAnnouncementByPeer.entries) {
            ...
            if (!mightContain(idBytes)) {
                ...
                val toSend = pkt.copy(ttl = com.skein.android.util.AppConstants.SYNC_TTL_HOPS)
    ...
    private fun buildGcsPayload(): ByteArray {
        ...
        val list = ArrayList<SkeinPacket>()
        // announcements
