...
import com.skein.android.services.SeenMessageStore
import com.skein.android.ui.ChatState
import com.skein.android.ui.MeshDelegateHandler
import com.skein.android.ui.PrivateChatManager
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: com.skein.android.ui.DataManager
) {
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch(Dispatchers.Default) {
            try {
                ...
                val packet = SkeinPacket.fromBinaryData(packetData) ?: return@launch

                if (packet.type != com.skein.android.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch
                ...
                com.skein.android.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
    ...
    private suspend fun processNoisePayload(
        ...
        when (payload.type) {
            ...
            NoisePayloadType.FILE_TRANSFER -> {
                ...
                val file = SkeinFilePacket.decode(payload.data)
                if (file != null) {
                    val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                    val savedPath = com.skein.android.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = SkeinMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
