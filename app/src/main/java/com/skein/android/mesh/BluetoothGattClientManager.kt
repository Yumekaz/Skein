...
import com.skein.android.util.AppConstants
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.delay
import kotlinx.coroutines.launch
import java.util.*
import kotlinx.coroutines.Job
import com.skein.android.ui.debug.DebugSettingsManager
import com.skein.android.ui.debug.DebugScanResult
...
class BluetoothGattClientManager(
    ...
    private fun isClientRoleEnabled(): Boolean {
        return isBleTransportEnabled() &&
            (try { com.skein.android.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value } catch (_: Exception) { true })
    }
    ...
    private fun handleScanResult(result: ScanResult) {
        ...
        val dbg = try { com.skein.android.ui.debug.DebugSettingsManager.getInstance() } catch (_: Exception) { null }
    ...
    private fun connectToDevice(device: BluetoothDevice, rssi: Int, peerID: String? = null) {
        ...
        val deviceAddress = device.address
        Log.i(TAG, "Connecting to Skein device: $deviceAddress (peerID: $peerID)")
        ...
        val gattCallback = object : BluetoothGattCallback() {
            ...
            override fun onCharacteristicChanged(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic) {
                ...
                val packet = SkeinPacket.fromBinaryData(value)
                if (packet != null) {
