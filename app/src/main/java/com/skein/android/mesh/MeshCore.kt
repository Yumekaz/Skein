...
import com.skein.android.model.PrivateMessagePacket
...
class MeshCore(
    ...
    data class Hooks(
        val onMessageReceived: ((SkeinMessage) -> Unit)? = null,
    ...
    init {
        ...
        if (sharedGossipManager == null) {
            gossipSyncManager.delegate = object : GossipSyncManager.Delegate {
                ...
                override fun sendPacketToPeer(peerID: String, packet: SkeinPacket) {
                    transport.sendPacketToPeer(peerID, packet)
                    TransportBridgeService.sendToPeer(transport.id, peerID, packet)
                }

                override fun signPacketForBroadcast(packet: SkeinPacket): SkeinPacket {
    ...
    fun processIncoming(packet: SkeinPacket, peerID: String?, relayAddress: String?) {
    ...
    private fun setupDelegates() {
        peerManager.delegate = object : PeerManagerDelegate {
            override fun onPeerListUpdated(peerIDs: List<String>) {
                try { com.skein.android.services.AppStateStore.setTransportPeers(transport.id, peerIDs) } catch (_: Exception) { }
    ...
    fun sendFileBroadcast(file: SkeinFilePacket) {
        try {
    ...
    fun sendFilePrivate(recipientPeerID: String, file: SkeinFilePacket) {
        try {
            scope.launch {
    ...
    fun sendVerifyChallenge(peerID: String, noiseKeyHex: String, nonceA: ByteArray) {
        val payload = NoisePayload(
            ...
            data = com.skein.android.services.VerificationService.buildVerifyChallenge(noiseKeyHex, nonceA)
        )
        sendNoisePayloadToPeer(payload, peerID)
    }

    fun sendVerifyResponse(peerID: String, noiseKeyHex: String, nonceA: ByteArray) {
        val tlv = com.skein.android.services.VerificationService.buildVerifyResponse(noiseKeyHex, nonceA) ?: return
        val payload = NoisePayload(
    ...
    private fun buildAnnouncementPayload(announcement: IdentityAnnouncement, nickname: String): ByteArray? {
        ...
        try {
            if (directPeersForGossip.isNotEmpty()) {
                tlvPayload += com.skein.android.services.meshgraph.GossipTLV.encodeNeighbors(directPeersForGossip)
            }
    ...
    private fun getDirectPeerIDsForGossip(): List<String> {
        return try {
            ...
            try { com.skein.android.services.AppStateStore.setTransportDirectPeers(transport.id, localDirect) } catch (_: Exception) { }
            val union = try {
                com.skein.android.services.AppStateStore.getDirectPeers().ifEmpty { localDirect }
        ...
        return try {
            val withRoute = try {
                ...
                if (recipient != null && !recipient.contentEquals(SpecialRecipients.BROADCAST)) {
                    ...
                    val path = com.skein.android.services.meshgraph.RoutePlanner.shortestPath(myPeerID, destination)
