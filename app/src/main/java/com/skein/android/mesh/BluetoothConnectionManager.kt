...
class BluetoothConnectionManager(
    ...
    private val componentDelegate = object : BluetoothConnectionManagerDelegate {
        override fun onPacketReceived(packet: SkeinPacket, peerID: String, device: BluetoothDevice?) {
    ...
    private fun isGattServerEnabled(): Boolean {
        return isBleTransportEnabled() &&
            (try { com.skein.android.ui.debug.DebugSettingsManager.getInstance().gattServerEnabled.value } catch (_: Exception) { true })
    }
    ...
    init {
        ...
        try {
            val dbg = com.skein.android.ui.debug.DebugSettingsManager.getInstance()
...
interface BluetoothConnectionManagerDelegate {
    fun onPacketReceived(packet: SkeinPacket, peerID: String, device: BluetoothDevice?)
