...
object TransportBridgeService {
    ...
    interface TransportLayer {
         ...
         */
        fun sendToPeer(peerID: String, packet: SkeinPacket) { }
    }
     ...
     */
    fun sendToPeer(sourceId: String, peerID: String, packet: SkeinPacket) {
        val targets = transports.filterKeys { it != sourceId }
        if (targets.isEmpty()) return
     ...
     */
    fun sendToPeerFromLocal(peerID: String, packet: SkeinPacket) {
        val targets = transports.toMap()
        if (targets.isEmpty()) return
    ...
    private fun prepareForwardedPacket(kind: String, packet: SkeinPacket): SkeinPacket? {
    ...
    private fun logicalPacketId(packet: SkeinPacket): String {
