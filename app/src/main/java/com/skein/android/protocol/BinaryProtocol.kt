package com.skein.android.protocol
...
data class SkeinPacket(
    ...
    fun toBinaryDataForSigning(): ByteArray? {
        ...
        val unsignedPacket = SkeinPacket(
            version = version,
            type = type,
            senderID = senderID,
            ...
            route = route,
            ttl = com.skein.android.util.AppConstants.SYNC_TTL_HOPS // Use fixed TTL=0 for signing to ensure relay compatibility
        )
        return BinaryProtocol.encode(unsignedPacket)
    }

    companion object {
        fun fromBinaryData(data: ByteArray): SkeinPacket? {
...
object BinaryProtocol {
    ...
    fun encode(packet: SkeinPacket, padding: Boolean = true): ByteArray? {
        try {
    ...
    fun decode(data: ByteArray): SkeinPacket? {
     ...
     */
    private fun decodeCore(raw: ByteArray): SkeinPacket? {
        try {
            ...
            if (payloadLength > com.skein.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH.toUInt()) {
                Log.w("BinaryProtocol", "Payload length ${payloadLength} exceeds maximum allowed (${com.skein.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH})")
                return null
            }
