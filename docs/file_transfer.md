# Skein Bluetooth File Transfer: Images, Audio, and Generic Files (with Interactive Features)

This document is the exhaustive implementation guide for Skein’s Bluetooth file transfer protocol for voice notes (audio) and images, including interactive features like waveform seeking. It describes the on‑wire packet format (both v1 and v2), fragmentation/progress/cancellation, sender/receiver behaviors, and the complete UX we implemented in the Android client so that other implementers can interoperate and match the user experience precisely.
...
- Protocol overview (SkeinPacket + File Transfer payload)
...
Skein BLE transport carries application messages inside the common `SkeinPacket` envelope. File transfer reuses the same envelope as public and private messages, with a distinct `type` and a TLV‑encoded payload.

### 1.1 SkeinPacket envelope
...
- `app/src/main/java/com/skein/android/mesh/BluetoothMeshService.kt`
- `app/src/main/java/com/skein/android/mesh/BluetoothConnectionManager.kt`
- `app/src/main/java/com/skein/android/mesh/PacketProcessor.kt`
...
### 1.3 File Transfer TLV payload (SkeinFilePacket)

The file payload is a TLV structure with mixed length field sizes to support large contents efficiently.

- Defined in `app/src/main/java/com/skein/android/model/SkeinFilePacket.kt`
...
- `app/src/main/java/com/skein/android/mesh/BluetoothPacketBroadcaster.kt`
- `app/src/main/java/com/skein/android/mesh/BluetoothConnectionManager.kt`
- `app/src/main/java/com/skein/android/mesh/BluetoothMeshService.kt`
- `app/src/main/java/com/skein/android/ui/ChatViewModel.kt`
...
- For both broadcast and private paths we try `SkeinFilePacket.decode(payload)`. If it decodes:
...
- `app/src/main/java/com/skein/android/mesh/MessageHandler.kt`
...
2) Local echo
   - We create a `SkeinMessage` with content `"[voice] <path>"` and add to the appropriate timeline (public/channel/private).
...
- `app/src/main/java/com/skein/android/ui/ChatViewModel.kt` (sendVoiceNote)
- `app/src/main/java/com/skein/android/model/SkeinFilePacket.kt`
- `app/src/main/java/com/skein/android/mesh/BluetoothMeshService.kt`
- `app/src/main/java/com/skein/android/features/voice/VoiceRecorder.kt`
- `app/src/main/java/com/skein/android/features/voice/Waveform.kt` (cache + extractor)
...
3) Packet creation
   - Build `SkeinFilePacket` with mime `image/jpeg` and file content.
...
- `app/src/main/java/com/skein/android/features/media/ImageUtils.kt`
- `app/src/main/java/com/skein/android/ui/ChatViewModel.kt` (sendImageNote)
- `app/src/main/java/com/skein/android/mesh/BluetoothMeshService.kt`
...
- `app/src/main/java/com/skein/android/ui/InputComponents.kt`
...
- While recording, a dense, real‑time scrolling waveform overlays the input showing live audio; a timer is shown to the right.
  - Component: `RealtimeScrollingWaveform` (dense bars, ~240 columns, ~20 FPS) in `app/src/main/java/com/skein/android/ui/media/RealtimeScrollingWaveform.kt`.
...
- `app/src/main/java/com/skein/android/ui/MessageComponents.kt`
- `app/src/main/java/com/skein/android/ui/media/WaveformViews.kt`
- `app/src/main/java/com/skein/android/features/voice/Waveform.kt`
...
- `app/src/main/java/com/skein/android/ui/media/ImagePickerButton.kt`
- `app/src/main/java/com/skein/android/features/media/ImageUtils.kt`
- `app/src/main/java/com/skein/android/ui/media/BlockRevealImage.kt`
- `app/src/main/java/com/skein/android/ui/MessageComponents.kt`
...
- `app/src/main/java/com/skein/android/ui/media/FullScreenImageViewer.kt`
...
Files:
- `app/src/main/java/com/skein/android/ui/MessageComponents.kt` — VoiceNotePlayer with seekTo function
- `app/src/main/java/com/skein/android/ui/media/WaveformViews.kt` — Interactive WaveformCanvas with tap handling
...
- `app/src/main/java/com/skein/android/model/SkeinFilePacket.kt` — TLV payload model + encode/decode.
- `app/src/main/java/com/skein/android/mesh/BluetoothMeshService.kt` — packet creation and broadcast for file messages.
- `app/src/main/java/com/skein/android/mesh/BluetoothPacketBroadcaster.kt` — fragmentation, progress, cancellation via transfer jobs.
- `app/src/main/java/com/skein/android/mesh/TransferProgressManager.kt` — progress events bus.
- `app/src/main/java/com/skein/android/mesh/MessageHandler.kt` — receive path: decode, persist to files, create chat messages.

Audio capture and waveform:

- `app/src/main/java/com/skein/android/features/voice/VoiceRecorder.kt` — MediaRecorder wrapper.
- `app/src/main/java/com/skein/android/features/voice/Waveform.kt` — cache + extractor + resampler.
- `app/src/main/java/com/skein/android/ui/media/WaveformViews.kt` — Compose waveform preview components.

Image pipeline:

- `app/src/main/java/com/skein/android/features/media/ImageUtils.kt` — downscale and save to app files.
- `app/src/main/java/com/skein/android/ui/media/ImagePickerButton.kt` — SAF picker button.
- `app/src/main/java/com/skein/android/ui/media/BlockRevealImage.kt` — block‑reveal progress renderer (no gaps, dense grid).

Recording overlay:

- `app/src/main/java/com/skein/android/ui/media/RealtimeScrollingWaveform.kt` — dense, real‑time scrolling waveform during recording.

UI composition and view model coordination:

- `app/src/main/java/com/skein/android/ui/InputComponents.kt` — input field, overlays (recording), picker button, mic.
- `app/src/main/java/com/skein/android/ui/MessageComponents.kt` — message rendering for text/audio/images including progress UIs and cancel overlays.
- `app/src/main/java/com/skein/android/ui/ChatViewModel.kt` — sendVoiceNote/sendImageNote, progress mapping, cancelMediaSend.
- `app/src/main/java/com/skein/android/ui/MessageManager.kt` — add/remove/update messages across main, private, and channels.

Fullscreen image:

- `app/src/main/java/com/skein/android/ui/media/FullScreenImageViewer.kt` — fullscreen viewer + save to Downloads.
...
2. Implement `SkeinFilePacket` TLV exactly as specified:
   - FILE_NAME and MIME_TYPE: `type(1) + len(2) + value`
   - FILE_SIZE: `type(1) + len(2=4) + value(4, UInt32 BE)`
   - CONTENT: `type(1) + len(4) + value`
3. Embed the TLV into a `SkeinPacket` envelope with `type = FILE_TRANSFER (0x22)` and the correct `recipientID` (broadcast vs private).
